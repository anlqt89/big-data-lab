...
export const Movies = () => {
    ...
    const [typeSearch, setTypeSearch] = useState("standard");
    ...
    const fetchData = async (cursor = "", isNewSearch = false, modeOverride = null) => {
        setLoading(true);
        
        // Determine settings based on state or override
        const activeMode = modeOverride || searchMode;
        if (activeMode === 'index'){
            setSearchMode("Index")
        }else if(activeMode === 'noIndex'){
            setSearchMode("No Index")
        }else if(activeMode === 'meterialized'){
            setSearchMode("Materialized")
        }
